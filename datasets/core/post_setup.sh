...
echo ""
docker --log-level ERROR compose exec spark-iceberg spark-shell --driver-memory 8g --conf spark.ui.enabled=false  -i /tmp/post_setup.scala 2>&1 >/dev/null
...
docker --log-level ERROR compose exec spark-iceberg spark-shell --driver-memory 8g --conf spark.ui.enabled=false  -i /tmp/check_transactions_count.scala 2>&1 >/dev/null
