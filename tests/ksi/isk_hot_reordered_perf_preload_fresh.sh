...
echo "  compose file: docker-compose.yaml:docker-compose.isk-hot.yaml:docker-compose.preload.yaml"
echo "Performance target records: ${REORDERED_PERF_TARGET_RECORDS:-1000000}"

cd "$ENV_DIR"
export COMPOSE_FILE=docker-compose.yaml:docker-compose.isk-hot.yaml:docker-compose.preload.yaml
docker --log-level ERROR compose up -d --force-recreate ksi

REORDERED_PERF_BASELINE_LABEL="ksi-hotset-preload-baseline" \
REORDERED_PERF_ORDERED_LABEL="ksi-hotset-preload-ordered" \
