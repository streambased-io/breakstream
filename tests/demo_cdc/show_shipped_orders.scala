...
spark.sql("" +
  "SELECT OrderID, Status as hotset_status FROM isk.hotset.orders WHERE Status = 'SHIPPED' ORDER BY OrderID ASC LIMIT 10" +
  "").show()
...
spark.sql("" +
  ...
  "ORDER BY o.OrderID ASC LIMIT 10" +
  "").show()
println(s"${GREEN}\nBut in the merged view, the hotset (SHIPPED) value wins:${RESET}")
spark.sql("" +
  "SELECT OrderID, Status as merged_status FROM isk.merged.orders WHERE Status = 'SHIPPED' ORDER BY OrderID ASC LIMIT 10" +
  "").show()
